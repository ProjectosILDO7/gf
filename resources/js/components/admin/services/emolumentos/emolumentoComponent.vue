<template>
    <div class="container mt-4">
        ...
        <topo-page-component :namePage="namePage" :nameButton="nameButto" :pageTopoIcon="pageTopoIcon"
            @buscar-curso-id="novoEmolumento" />

        <br>

        <div class="row">
            <div class="form-group col-6 text-start mb-2">
                <span class="text-secondary">Total de emolumentos:</span> <span class="fw-bold">( {{ totalDeEmolumentos }}
                    )</span>
            </div>

            <div class="form-group col-6 text-end mb-2" v-if="totalDeEmolumentos != 0">
                <a class="btn btn-sm btn-outline-success ml-2" :href="`${urlExport}${meID}`">
                    <i class="fa-solid fa-file-excel fa-lg"></i> Exportar lista de emolumentos
                </a>
            </div>
        </div>


        <form class="d-flex" role="search" v-if="totalDeEmolumentos >= 6">
        ...
        <div class="row" v-if="emolumentos == ''">
            <div class="form-group col-12 text-danger text-center fw-bold">
                De momento não tens nenhum emolumento cadastrado...
            </div>
        </div>

        <div class="card mt-2 shadow" v-for="emolumento in emolumentos" :key="emolumento.id">
            <div class="card-body">
                <div class="row">
                            ...
                            <div class="dropdown">
                                ...
                                <ul class="dropdown-menu">
                                    <li><a class="dropdown-item" href="#" data-bs-toggle="modal"
                                            data-bs-target="#staticBackdrop" @click="updateEmolumentoForm(emolumento.id)"><i
                                                class="fa-regular fa-pen-to-square text-success"></i> Alterar</a></li>
                                    <li><a class="dropdown-item" href="#" data-bs-toggle="modal"
                                            data-bs-target="#modalDeleteConfirm" @click="deleteEmolumento(emolumento.id)"><i
                    ...
                    <div class="form-group col-xs-12 col-sm-12 col-md-4 col-lg-4">
                        <label class="text-success">Emolumento</label>
                        <p class="text-secondary h6"><i class="fa-solid fa-id-card-clip text-danger"></i> {{
                    ...
                    <div class="form-group col-xs-12 col-sm-12 col-md-4 col-lg-4">
                        ...
                        <p class="text-secondary h6"><i class="fa-solid fa-cart-shopping"></i>
                             {{ vueNumberFormat(emolumento.cobranca, {isInteger:true})}}
                        </p>
                    </div>
        ...
        <nav aria-label="paginate mt-4">
            <ul class="pagination pagination-sm" v-if="emolumentos.last_page > 1">
                <li class="page-item" v-if="emolumentos.current_page != 1">
                    <a class="page-link" href="#" @click.prevent="loadingEmolumentos(emolumentos.current_page - 1)">{{ '<<'
                    }}</a>
                </li>

                <li :class="['page-item', { 'active': emolumentos.current_page == page }]" aria-current="page"
                    v-for="page in emolumentos.last_page" :key="page.id">
                    <a class="page-link" href="#" @click.prevent="loadingEmolumentos(page)">{{ page }}</a>
                </li>

                <li class="page-item" v-if="emolumentos.current_page < emolumentos.last_page">
                    <a class="page-link" href="#" @click.prevent="loadingEmolumentos(emolumentos.current_page + 1)">{{ '>>'
                    }}</a>
                </li>
            </ul>
            ...
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="staticBackdropLabel">
                            <span class="" v-if="btnSaveVariavel">
                                <i class="fa-solid fa-graduation-cap"></i> Cadastre novo emolumento
                            </span>
                            <span class="" v-else>
                                <i class="fa-solid fa-graduation-cap"></i> Alter informações do emolumento
                            </span>
                        ...
                        <div class="row">
                            <form action="">
                                ...
                                <div class="form-group col-12 mb-2">
                                    <span class="text-danger small col-12" v-if="erros.emolumento">{{ erros.emolumento[0]
                                    }}</span>
                                    <label for="" class="text-secodary col-12">Nome do emolumento</label>
                                    <input type="text" class="form-control form-control-sm"
                                        placeholder="Informe o emolumento" v-model="items.emolumento">
                                </div>
                    ...
                    <div class="modal-footer" v-if="btnSaveVariavel">
                        ...
                        <button @click.prevent="registerEmolumento" type="button" class="btn btn-sm btn-success">
                            <span v-if="loading">
                    ...
                    <div class="modal-footer" v-else>
                        ...
                        <button @click.prevent="updateEmolumento" type="button" class="btn btn-sm btn-success">
                            <span v-if="loading">
                                <i class="fa-regular fa-floppy-disk"></i> Tentando salvar...
                            </span>
                            <span v-else>
            ...
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                        ...
                        <div class="row">
                            <div class="form-group col-12 text-center">
                                <span class="text-primary">
                                    <h6>Tens a certeza que desejas apagar este emolumento...?</h6>
                                </span>
                            </div>
                        </div>
                    </div>
                    <div class="modal-footer">
                        ...
                        <button @click.prevent="apagarEmolumento(deleteEmolumentoId)" type="button"
...
export default {
    ...
    data() {
        return {
            urlExport: URL_API + '/ExportToExcelEmolumento/',
            meID:'',
            items: { emolumento:'', cobranca:'' },
            erros: [],
            deleteEmolumentoId: '',
            filter: '',
            info: [],
            fomatoData: '',
            namePage: 'Emolumentos',
            nameButto: 'Novo emolumento',
            pageTopoIcon: 'fa-solid fa-id-card-clip',
            loading: false,
            btnSaveVariavel: false,
        }
    },
    created() {
        this.loadingEmolumentos()
        this.me()
    },

    computed: {
        emolumentos() {
            return this.$store.getters.todosEmolumentos(this.filter)
        },

        totalDeEmolumentos(){
            return this.$store.getters.todosEmolumentosCount
        },
    ...
    methods: {
        ...
        detalhes(id) {
            this.info = []
            this.$store.dispatch('detalhesEmolumento', id)
                .then((response) => {
        ...
        cleanForm() {
            this.items = { emolumento: '', cobranca:'' },
            this.erros = []
        },

        loadingEmolumentos() {
            //var store = useStore()
            this.$store.dispatch('loadingEmolumentos')
        },

        registerEmolumento() {
            this.$store.dispatch('createEmolumento', this.items)
                .then(() => {
                    notify({
                        title: 'Sucesso',
                        text: "O Emolumento foi registado com sucesso",
                        type: 'success'
                    })
                    this.cleanForm()
                    this.loadingEmolumentos()
        ...
        updateEmolumentoForm(id) {
            this.btnSaveVariavel = false
            this.cleanForm()
            this.$store.dispatch('updateFormEmolumento', id)
                .then((response) => this.items = response.data.getEmolumento)
                .catch((error) => {
                    notify({
                        title: 'Não encotrado',
                        text: 'O Emolumento que pretendes localizar não existe',
                        type: 'warn'
                    })
                })
        },
        ...
        updateEmolumento() {
            this.$store.dispatch('updateEmolumento', this.items)
                .then(() => {
                    notify({
                        title: 'Sucesso',
                        text: 'O emolumento foi actualizado com sucesso..!',
                        type: 'success'
                    })
                    this.cleanForm()
                    this.loadingEmolumentos()
                ...
                .catch((error) => {
                    notify({
                        title: 'Erro',
                        text: 'Não foi possível actualizar este emolumento',
                        type: 'warn'
                    })
        ...
        deleteEmolumento(id) {
            this.deleteEmolumentoId = id;
        },

        apagarEmolumento(id) {
            this.$store.dispatch('apagarEmolumento', id)
                .then((response) => {
                    notify({
                        title: 'Sucesso',
